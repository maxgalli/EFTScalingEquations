    ...
    for channel, file in channel_file.items():
        ...
        for edges, deviations in zip(edges_list, bins):
            ...
            for dev in deviations:
                ...
                pars = dev[2:]
                if len(pars) == 1:
                    output_dict[channel][bin_name]["A_{}".format(pars[0])] = 10.1
                    output_dict[channel][bin_name]["u_A_{}".format(pars[0])] = 0.1
                elif len(pars) == 2:
                    if pars[0] == pars[1]:
                        output_dict[channel][bin_name]["B_{}_2".format(pars[0])] = 10.1
                        output_dict[channel][bin_name]["u_B_{}_2".format(pars[0])] = 0.1
                    else:
                        output_dict[channel][bin_name][
